...
describe("resolveMemoryRoleSlot", () => {
  it("preserves legacy plugins.slots.memory for recall when canonical recall is absent", () => {
    const cfg = {
      plugins: {
        slots: {
    ...
    expect(resolveMemoryRoleSlot({ cfg, role: "recall" })).toBe("legacy-memory");
    expect(listConfiguredMemoryRoleSlotSelections({ cfg })).toEqual([
      { role: "recall", slotKey: "memory.recall", pluginId: "legacy-memory" },
    ]);
  });

  it("uses legacy plugins.slots.memory=none as disabled recall when canonical recall is absent", () => {
    const cfg = {
      plugins: {
        slots: {
          memory: "none",
    ...
    expect(resolveMemoryRoleSlot({ cfg, role: "recall" })).toBeNull();
    expect(listConfiguredMemoryRoleSlotSelections({ cfg })).toEqual([]);
  });

  it("uses canonical memory.recall when legacy memory conflicts", () => {
    const cfg = {
      plugins: {
        slots: {
          memory: "legacy-memory",
    ...
    expect(resolveMemoryRoleSlot({ cfg, role: "recall" })).toBe("canonical-memory");
    expect(listConfiguredMemoryRoleSlotSelections({ cfg })).toEqual([
      { role: "recall", slotKey: "memory.recall", pluginId: "canonical-memory" },
    ]);
  });

  it("keeps canonical memory.recall taking precedence over legacy memory=none", () => {
    const cfg = {
      plugins: {
        slots: {
          memory: "none",
  ...
  it("preserves explicit per-agent memory role disablement as a negative selection", () => {
    const cfg = {
    ...
    expect(resolveMemoryRoleSlot({ cfg, role: "recall", agentId: "research" })).toBeNull();
    expect(resolveMemoryRoleSlot({ cfg, role: "recall", agentId: "writer" })).toBe("global-recall");
    expect(listConfiguredMemoryRoleSlotSelections({ cfg })).toEqual([
  ...
  it("preserves per-agent legacy memory selectors for recall", () => {
    const cfg = {
      plugins: { slots: { "memory.recall": "global-recall" } },
      agents: {
        list: [
          {
            id: "research",
            plugins: { slots: { memory: "legacy-agent-recall" } },
          },
        ],
      },
    } as OpenClawConfig;
