...
function resolveLegacyGatewayStartupMemoryPluginId(
  config: OpenClawConfig,
  options: { agentId?: string } = {},
): string | undefined {
  const readLegacySlot = (slots: unknown): string | undefined => {
    if (!isRecord(slots) || !Object.hasOwn(slots, "memory")) {
      return undefined;
    }
    // Canonical role slots take precedence; only use the legacy alias as a fallback.
    if (Object.hasOwn(slots, "memory.recall") || Object.hasOwn(slots, "memory.dreaming")) {
      return undefined;
    }
    const normalized = normalizeOptionalLowercaseString(slots.memory);
    return normalized && normalized !== "none" ? normalized : undefined;
  };

  if (options.agentId) {
    const agent = config.agents?.list?.find(
      (candidate) => candidate.id?.trim() === options.agentId,
    );
    const agentSlot = readLegacySlot(agent?.plugins?.slots);
    if (agentSlot) {
      return agentSlot;
    }
  }
  return readLegacySlot(config.plugins?.slots);
}
...
function resolveGatewayStartupDreamingSelectedPluginId(
  ...
  return resolveLegacyGatewayStartupMemoryPluginId(config, options);
}
...
function listGatewayStartupMemorySlotSelections(params: {
  activationSourceConfig: OpenClawConfig;
  normalizePluginId: (pluginId: string) => string;
}): MemoryRoleSlotSelection[] {
  const selections = listConfiguredMemoryRoleSlotSelections({
    cfg: params.activationSourceConfig,
  }).flatMap((selection) => {
    const pluginId = params.normalizePluginId(selection.pluginId);
    return pluginId && pluginId.toLowerCase() !== "none" ? [{ ...selection, pluginId }] : [];
  });
  const addLegacyRecallSelection = (slots: unknown, agentId?: string) => {
    if (!isRecord(slots) || !Object.hasOwn(slots, "memory")) {
      return;
    }
    // The role resolver intentionally ignores the legacy memory slot, but gateway startup must
    // still load a legacy-selected recall plugin for backward compatibility unless the canonical
    // memory.recall slot is configured at the same scope.
    if (Object.hasOwn(slots, "memory.recall")) {
      return;
    }
    const rawPluginId = slots.memory;
    if (typeof rawPluginId !== "string") {
      return;
    }
    const pluginId = params.normalizePluginId(rawPluginId);
  ...
  addLegacyRecallSelection(params.activationSourceConfig.plugins?.slots);
  for (const agent of params.activationSourceConfig.agents?.list ?? []) {
    const agentId = agent.id?.trim();
    if (!agentId) {
      continue;
    }
    addLegacyRecallSelection(agent.plugins?.slots, agentId);
  }
  return selections;
}
...
function resolveMemorySlotStartupPluginIds(params: {
  ...
  for (const selection of listGatewayStartupMemorySlotSelections({
    activationSourceConfig,
    normalizePluginId,
  })) {
  ...
  if (
    includeDefaultRecallSlot &&
    !Object.hasOwn(rawSlots ?? {}, "memory.recall") &&
    !Object.hasOwn(rawSlots ?? {}, "memory")
  ) {
