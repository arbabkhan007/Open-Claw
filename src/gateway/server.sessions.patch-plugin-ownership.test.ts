...
test("sessions.patch rejects plugin-runtime patches when a foreign-owned row appears under the write lock", async () => {
  const { storePath } = await createSessionStoreDir();
  ...
  // Simulate the race where the foreign-owned row is not visible to the
  // pre-transaction load but is present when the write lock selects the row.
