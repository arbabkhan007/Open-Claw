...
import { parseManagedOutgoingImageRelativeUrl } from "./managed-image-attachment-route.js";
...
type ManagedOutgoingImageDownload = {
  data: Buffer;
  contentType: string;
  sizeBytes: number;
...
export async function readManagedOutgoingImageDownloadUrl(params: {
  ...
  const route = parseManagedOutgoingImageRelativeUrl(params.url);
  if (!route || route.sessionKey !== params.expectedSessionKey) {
    return null;
  }
