...
export function listCommandPromptSurfaces(
  ...
  const pluginSurfaces = (params.includeHostCli === false ? [] : (params.pluginCommands ?? []))
    ...
    params.scope === "node-operator"
      ? (params.nodeCommands ?? [])
          ...
          .map((command) => ({
            ...
            commandHints: [
              [
                `nodes action=invoke node=${command.nodeId ?? command.nodeName ?? "selected"}`,
  ...
  return [
    ...(params.includeHostCli === false ? [] : listRoutedCommandSurfaces()),
    ...dynamicSurfaces,
  ];
}
