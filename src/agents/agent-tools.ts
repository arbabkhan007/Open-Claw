...
export function createOpenClawCodingTools(options?: {
  ...
  if (shouldInheritEffectiveToolAllowlist) {
    // Concrete parent tools drop deferred MCP/LSP/plugin selectors that only
    // materialize at runtime; re-filter those tokens through the SAME layers that
    // produced subagentFiltered so the child rebuilds them without escalating. (#85030)
