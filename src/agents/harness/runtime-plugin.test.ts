...
describe("ensureSelectedAgentHarnessPlugin", () => {
  ...
  it("activates a legacy-only plugins.slots.memory plugin for harness scoped loads", async () => {
    mocks.loadPluginRegistrySnapshot.mockReturnValueOnce(
      createPluginRegistrySnapshot(["some-memory-plugin"]),
    );
  ...
  it("activates an agent-scoped legacy-only plugins.slots.memory plugin", async () => {
    mocks.loadPluginRegistrySnapshot.mockReturnValueOnce(
      createPluginRegistrySnapshot(["some-memory-plugin"]),
    );
      ...
      config: {
        agents: {
          list: [
            {
              id: "agent-a",
              plugins: { slots: { memory: "some-memory-plugin" } },
            },
          ],
        },
        plugins: {
