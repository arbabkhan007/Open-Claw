...
describe("dispatchReplyFromConfig", () => {
  ...
  it("aborts plugin-owned binding delivery during route await via abort signal", async () => {
    ...
    // We abort synchronously inside the mock so the abort fires AFTER the pipeline
    // isPreDispatchOperationAborted checks but BEFORE deliverBindingPayload's
    // post-check, verifying the post-check prevents fallback delivery.
    const originalRouteReply = mocks.routeReply.getMockImplementation();
    mocks.routeReply.mockImplementation(async (_params: unknown) => {
      abortController.abort();
      // as unknown as object avoids type-suppression-inventory ("no as any");
      // the cast satisfies TypeScript while returning null at runtime,
      // which makes deliverBindingPayload's `if (result)` fall through to the
      // post-abort guard (the exact race window this test covers).
