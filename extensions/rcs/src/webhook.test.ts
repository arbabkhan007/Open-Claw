...
import { parseTwilioFormBody } from "./twilio.js";
...
function createTestReplayGuard() {
  return createChannelReplayGuard<{ accountId: string; messageSid: string }>({
    dedupe: { ttlMs: 10 * 60_000, memoryMaxSize: 10_000 },
...
function createTestRcsWebhookHandler(
  params: Parameters<typeof createRcsWebhookHandler>[0],
  deps: NonNullable<Parameters<typeof createRcsWebhookHandler>[1]> = {},
) {
  return createRcsWebhookHandler(params, { ...deps, replayGuard: createTestReplayGuard() });
}

function createTestRcsSharedWebhookHandler(
  params: Parameters<typeof createRcsSharedTwilioWebhookHandler>[0],
  deps: NonNullable<Parameters<typeof createRcsSharedTwilioWebhookHandler>[1]> = {},
) {
  return createRcsSharedTwilioWebhookHandler(params, {
    ...deps,
    replayGuard: createTestReplayGuard(),
  });
}
