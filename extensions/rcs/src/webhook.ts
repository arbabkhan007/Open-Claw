...
import {
  ...
  rcsWebhookReplayGuard,
  type RcsWebhookRateLimiter,
  type RcsWebhookReplayGuard,
...
type RcsWebhookHandlerDeps = {
  ...
  replayGuard?: RcsWebhookReplayGuard;
};
...
async function dispatchVerifiedRcsForm(params: {
  ...
  if (
    !(await params.replayGuard.shouldProcess({
...
export function createRcsWebhookHandler(
  ...
  const replayGuard = deps.replayGuard ?? rcsWebhookReplayGuard;
