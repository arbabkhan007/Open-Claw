...
// The registered-tools build pass ignores the runtime plan, so
// normalizeAgentRuntimeTools falls back to the provider runtime plugin, and
// loading that registry in a cold vitest worker compiles bundled provider
// plugins for over a minute, starving the test timeout. This test pins yield
// lifecycle meta, not provider schema normalization, so keep normalization an
// identity pass-through.
vi.mock("openclaw/plugin-sdk/agent-harness-runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("openclaw/plugin-sdk/agent-harness-runtime")>()),
  normalizeAgentRuntimeTools: <T>(params: { tools: T[] }) => params.tools,
}));
