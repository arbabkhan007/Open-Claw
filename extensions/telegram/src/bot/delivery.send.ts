...
// Telegram reports empty-content rejections with three wordings: the legacy
// "message text is empty", the newer Bot API "text must be non-empty", and
// RICH_MESSAGE_CONTENT_REQUIRED on the rich-message lane. Match all so
// render-to-empty payloads never surface as delivery failures.
const EMPTY_TEXT_ERR_RE =
  /message text is empty|text must be non-empty|RICH_MESSAGE_CONTENT_REQUIRED/i;
...
export async function sendTelegramText(
  ...
  if (opts?.richMessages === true && textMode !== "html") {
    ...
    try {
      ...
      if (!hasFallbackText && EMPTY_TEXT_ERR_RE.test(formatErrorMessage(err))) {
        runtime.log?.(
  ...
  try {
    ...
    const errText = formatErrorMessage(err);
    const isEmptyTextError = EMPTY_TEXT_ERR_RE.test(errText);
