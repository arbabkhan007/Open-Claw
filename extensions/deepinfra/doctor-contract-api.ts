...
export function normalizeCompatibilityConfig({ cfg }: { cfg: OpenClawConfig }): {
  ...
  if (Object.hasOwn(next, "nativeBaseUrl")) {
    ...
    if (normalizeBaseUrlValue(next.baseUrl)) {
      ...
      changes.push(
        `${PROVIDER_PATH}.nativeBaseUrl: retired custom native endpoint ${legacyNative}; using ${CANONICAL_BASE_URL} - set ${PROVIDER_PATH}.baseUrl manually if your host serves an OpenAI-compatible videos API`,
      );
    } else {
      ...
      changes.push(`${PROVIDER_PATH}.nativeBaseUrl -> ${PROVIDER_PATH}.baseUrl (${migrated})`);
    }
  }
  ...
  if (
    ...
    const migrated = next.baseUrl.replace(NATIVE_INFERENCE_PATH, OPENAI_COMPAT_PATH);
    next.baseUrl = migrated;
    changes.push(
      `${PROVIDER_PATH}.baseUrl: ${NATIVE_INFERENCE_PATH} -> ${OPENAI_COMPAT_PATH} (${migrated})`,
    );
  }
