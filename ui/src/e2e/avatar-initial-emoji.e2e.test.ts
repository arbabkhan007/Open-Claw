...
describeControlUiE2e("Control UI grapheme-aware avatar initials", () => {
  ...
  it("renders the emoji grapheme initial in the sidebar chip and agent menu row", async () => {
    ...
    try {
      ...
      // Open the sidebar agent-chip menu and inspect the emoji agent row's
      ...
      // Switch to the emoji agent; the sidebar chip avatar text fallback
      // (app-sidebar chipAvatarText) must keep the full grapheme cluster.
      ...
      await expect
        ...
        .toBe(true);
      await expect
        .poll(() => sidebar.locator(".sidebar-agent-chip__avatar-text").textContent())
        .toBe(emojiGrapheme);
