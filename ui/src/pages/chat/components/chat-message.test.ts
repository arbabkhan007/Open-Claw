...
describe("grouped chat rendering", () => {
  ...
  it("falls back gracefully when a managed outgoing image body stalls after headers arrive", async () => {
    vi.useFakeTimers();
    const managedChatImageUrl = `/api/chat/media/outgoing/agent%3Amain%3Amain/${crypto.randomUUID()}/full`;
    const fetchMock = vi.fn(async (_url: string, init?: RequestInit) => {
      const signal = init?.signal;
        ...
        blob: async () =>
          await new Promise<Blob>((_resolve, reject) => {
            signal.addEventListener(
              "abort",
              () => {
                // Simulate native body-read abort behavior: the rejection is an
                // AbortError, not the custom TimeoutError carried by signal.reason.
