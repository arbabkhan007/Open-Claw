...
| `OTEL_SEMCONV_STABILITY_OPT_IN`                                                                                   | Set to `gen_ai_latest_experimental` to emit the latest GenAI inference span shape: `{gen_ai.operation.name} {gen_ai.request.model}` span names and `gen_ai.provider.name` instead of the legacy `gen_ai.system`. Model-call spans use `CLIENT` regardless of this setting; generic tool-execution spans remain `INTERNAL` because they may represent local work. GenAI metrics always use bounded, low-cardinality attributes regardless. |
...
- `openclaw.model.call`
  ...
  - Model-call spans use `CLIENT` span kind. With `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental`, they use the latest GenAI inference span name `{gen_ai.operation.name} {gen_ai.request.model}` instead of `openclaw.model.call`; the span kind does not depend on the opt-in.
...
- `openclaw.tool.execution`
  - `INTERNAL` span kind because generic tool events may describe local work; transport-specific instrumentation can emit `CLIENT` child spans when it knows a remote call occurred
