...
| **`hot`**              | Hot-applies safe changes only. Logs a warning when a restart is needed; you handle it. Security-critical changes (`gateway.auth.*`, `auth.profiles.*`/`auth.order.*`, `secrets.providers.*`) still auto-restart. |
