...
- `channels.slack.appHome.viewPath` points to a Slack Block Kit Home view JSON file that OpenClaw reads each time Slack opens the Home tab, so copy edits do not require a Gateway restart. `channels.slack.appHome.view` can hold the same Home view inline. `viewPath` takes precedence when both are configured. The view must be type `"home"` (or omit `type`, which OpenClaw fills as `"home"`) and contain 1-100 blocks. Invalid files or inline views are logged and fall back to the built-in safe Home view. Account-level `channels.slack.accounts.<accountId>.appHome` overrides the top-level value for that account. Do not include secrets or private operational details; Slack shows the Home tab to workspace members who can open the app.
