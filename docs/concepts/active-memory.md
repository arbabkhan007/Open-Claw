...
Then save the file. OpenClaw hot-reloads plugin selection; restarting the gateway is not required:

```bash
# If OpenClaw is already running, it notices the config change automatically.
# New turns use the selected memory.recall provider once the reload completes.
```
