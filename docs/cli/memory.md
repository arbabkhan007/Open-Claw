...
- `--agent <id>`: scope to a single agent (default: the default agent).
- `--path <file-or-dir>`: seed the harness from historical daily memory file(s).
...
Write grounded historical REM summaries into `DREAMS.md` for UI review, with optional short-term promotion staging. Backfill writes are reversible.

```bash
openclaw memory rem-backfill --path <file-or-dir> [--grounded] [--stage-short-term] [--agent <id>] [--json]
openclaw memory rem-backfill [--rollback] [--rollback-short-term] [--agent <id>] [--json]
```

- `--agent <id>`: scope to a single agent (default: the default agent).
- `--path <file-or-dir>`: read historical daily memory file(s), or a directory containing `YYYY-MM-DD.md` daily notes. Required unless using a rollback mode.
- `--grounded`: accepted for clarity; `rem-backfill` writes grounded output by default.
- `--stage-short-term`: also stage grounded durable candidates into the live short-term promotion store so normal deep promotion can rank them later. Existing grounded backfill candidates are replaced before staging new ones.
- `--rollback`: remove previously written grounded diary entries from `DREAMS.md`/`dreams.md`.
- `--rollback-short-term`: remove previously staged grounded short-term candidates from the promotion store.
- `--json`: print JSON output including workspace/source paths, written/replaced diary counts, and staged/removed short-term counts when applicable.

Modes and outputs:

- Path mode (`--path <file-or-dir>`) scans historical daily notes, renders grounded REM backfill entries, and writes reversible diary entries into `DREAMS.md` (or existing `dreams.md`) for UI review.
- Staging mode (`--path <file-or-dir> --stage-short-term`) also seeds grounded durable candidates into the short-term promotion store.
- Rollback mode (`--rollback`) removes previously written grounded diary entries.
- Short-term rollback mode (`--rollback-short-term`) removes previously staged grounded short-term candidates.
