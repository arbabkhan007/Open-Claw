...
openclaw channels add --channel telegram --token <bot-token> --dm-policy disabled
...
Non-interactive add flags shared across channels: `--account <id>`, `--name <name>`, `--token`, `--token-file`, `--bot-token`, `--app-token`, `--secret`, `--secret-file`, `--password`, `--dm-policy <open|pairing|allowlist|disabled>`, `--cli-path`, `--url`, `--base-url`, `--http-url`, `--auth-dir`, and `--use-env` (env-backed auth, default account only, where supported). A channel that does not expose DM policy setup rejects `--dm-policy` instead of silently ignoring it. Sender allowlist inputs remain channel-specific; for example, Tlon owns `--dm-allowlist` and its ship-name normalization. Set a non-empty channel-specific sender allowlist before selecting `allowlist`; restrictive policies reject an existing `"*"` wildcard until you remove it through that channel's configuration.

Channel-specific flags include:
