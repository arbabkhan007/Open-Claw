...
6. From the intended clean commit, run `pnpm android:screenshots` to refresh phone and Wear Google Play screenshots with the two script-managed emulators and per-locale capture manifests.
7. Run `pnpm android:release:archive` to produce the signed phone Play AAB, Wear AAB, third-party APK, checksums, and `apps/android/build/release-artifacts/release-manifest.json`.
8. Run `pnpm android:release:upload` to validate the current SHA/version/code/hash provenance, then upload metadata, screenshots, the phone AAB, and the Wear AAB to their phone and `wear:` tracks in one atomic Google Play edit. If provenance is missing or stale, rerun the archive step from the intended clean commit.
